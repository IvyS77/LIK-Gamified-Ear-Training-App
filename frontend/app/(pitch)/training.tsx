...
async function applyAfterCheck(params: { uid: string; correct: boolean; mode: 'demo' | 'full'; }) {
  ...
  let currentXp: number = Number(data.currentXp ?? 0);
  ...
  if (correct) {
    ...
    const intended = Math.max(1, Math.round(BASE_XP_PER_CORRECT * mult));
    if (mode === 'demo') {
  ...
  await setDoc(
    ref,
    {
      currentXp, level, streak, lastPlayedDate: today,
      recentCorrect, recentTotal, demoXpToday, demoXpDate: today,
...
export default function TrainingScreen() {
  ...
  const theme = useMemo(() => {
    ...
    return {
      ...
      whiteTop: isDarkPiano ? '#F4F7FB' : '#FFFFFF',
      whiteBorder: isDarkPiano ? '#8794B5' : '#D8E6FF',
      whiteDepth: isDarkPiano ? '#CFD7E5' : '#D9DDE7',

      blackTop: isDarkPiano ? '#212734' : '#111827',
      blackBorder: isDarkPiano ? 'rgba(255,255,255,0.07)' : 'rgba(35,42,55,0.08)',
      blackDepth: isDarkPiano ? '#44506B' : '#222C38',
      blackText: '#FFFFFF',
  ...
  const onCheck = async () => {
    ...
    if (!user) return;
    const res = await applyAfterCheck({ uid: user.uid, correct, mode });
                          ...
                          >
                            <Text
                              style={[
                                ...
                                { color: selected ? '#FFFFFF' : (isDark ? '#222C38' : theme.text) },
                              ]}
                            >
                              {note}
                            </Text>
...
const styles = StyleSheet.create({
  ...
  listenBtn: {
    height: 48, borderRadius: 18, alignItems: 'center',
